...
function mBlocks(m) {
    $(m).map(function () {
        ...
        let el = $(this),//-e
            ...
            li = el.attr("data-feed") || "https://mohanjeet.blogspot.com",//blogspot site url - default or custom
            cor = (el.attr("data-corner")||"round").toLowerCase(),
...
const isH=(-1!=t.search("h")),isI=(-1!=t.search("i")),isS=(-1!=t.search("s")),isA=(-1!=t.search("a")),isCTA = (typeof (cta) !== "undefined"), isMo = ((typeof (mo) !== "undefined"));
(bP<=1)&&(isCa=false);
...
(bTy=="v")&&(bG="");
        ...
        //console.log(el);
//           console.log({tit,li,cor,thm,tva});
...
//isCa&&console.log(aC);
//console.log({tit,bC,bP,bR,l,toRe,isCa,isNav});
...
$(cI).addClass('carousel-indicators').appendTo($(a1));
...
let hD=hN="";
(fTy=="q")?(hN='<blockquote class="blockquote m-5">' + ti + '</blockquote>'):((isH)&&(hD='<h3 class="display-5 mx-lg-5 '+(isL?"opacity-50":"opacity-75")+'">' + ti + '</h3>',hN='<h5 class="card-title fw-normal '+((fTy=="t"||fTy=="l"||fTy=="p")?'text-' + iThm:"")+'">' + ti + '</h5>'));

//SNIPPET & COMMENT AUTHOR
(fTy=="q")&&(sni += '<figcaption class="blockquote-footer m-5">' + au + '</figcaption>');
let c="",z="";
...
//LINK
for (var i = 0; i < it.link.length; i++) if ("alternate" == it.link[i].rel) {const li = it.link[i].href; break; }
...
(fTy=="q")&&(iS+='height:6rem;width:6rem;',iSF+='height:6rem;width:6rem;',iC='rounded-circle');
iC+=' '+cor;
...
if(isF){im='<figure class="'+iC+' m-0" style="'+iSF+'" role="img" loading="lazy" aria-label="' + ti + ' image"'+t+'></figure>';}
...
//CAROUSEL INDICATORS
(isCa&&(p%(aC*bR)==0))&&($(cI).append('<button type="button" data-bs-target="#m' + mID + '" data-bs-slide-to="' +p/(aC*bR)+ '" '+(p==0?('class="active" aria-current="true"'):'')+' aria-label="Slide '+(p/(aC*bR)+1)+'"></button>'));
//(p<(bP-1)||(fTy=="v"&&(p<l-1)))

//SHOWCASE
if(isC){
if (fTy=="s"&&p==0) {
b+= '<div class="feature-image card border-0 text-center bg-' + thm + ' '+((cor == "")?(' rounded-5 rounded-bottom'):cor)+' overflow-hidden"><div class="sIframe" style="display:none;"></div>'+im+'<a class="link-' + iThm + '" href="' + li + '" title="' + ti + '">';
(isH)&&(b+= '<div class="sContent card-img-overlay rounded-0 '+(cor==""&&"rounded-top")+' mx-md-5 p-3 px-lg-5 bg-' + thm + ' bg-opacity-75 mt-auto" style="height:fit-content;">' + hN +' '+ sni + '</div>');
b+=ctaF;
b+= '</a></div>';
}
}
...
isC&&((fTy=="t")&&(b+=' vstack'));
(fTy!="v")&&(!(isC&&(fTy=="t"||fTy=="c"))&&(b+=' my-3'),(isCa||isNav)&&(b +=' px-2 px-sm-3 px-md-4 px-lg-5'));
switch (fC){
...
if(fTy!="s"){
//LINK START
b += '<a class="h-100'+(fTy=="t"?" row g-0 ":"")+'" href="' + li + '" title="' + ti + '">';
}

//IMAGE
if(isI){
(fTy=="q")&&(b +='<figure class="mx-auto">');
(fTy=="t")&&(b +='<figure class="col-4 m-0">');
b+=im;
//console.log({tit,im});
(fTy=="q"||fTy=="t")&&(b +='</figure>');
}
...
switch (fTy) {
case"v":b += '<div class="card-img-overlay p-3 px-lg-5 pb-5 text-center rounded-0 d-flex">';
case "g":
case"q":case "c":b += '<div class="card-img-overlay text-bg-' + thm + ' bg-opacity-75 rounded-0">';break;
case"p":b += '<div class="card-body bg-' + thm + ' bg-opacity-75">';break;
...
//NAV
                        if (isNav) {
                           m.find(".nav-prev").unbind('click').click(function () {
                                ...
                                m.find(".st"+s).fadeOut(); m.find(".st"+ (s - 1)).fadeIn();
                            });
                            m.find(".nav-next").unbind('click').click(function () {
                              const s = (el.attr("data-s"));
                                m.find(".st"+s).fadeOut();
el.attr("data-s", +s+1);
t=m.find(".st"+(+s+1)).length;
console.log({s,t,b},t==0,t[1],t[0],t[2]);
(t==0)?(mBlocks(el)):m.find(".st"+(+s+1)).fadeIn();
                            });
                        }//if
//SHOWCASE
