...
export function renderCTA(finalType, config, postTitle, postUrl) {
    ...
    const baseClasses = `inline-block text-label-lg font-bold no-underline px-4 py-2 hover:opacity-90 transition-opacity opacity-90 ${config.theme.bg} ${config.theme.text} ${isSharp ? 'rounded-none' : 'rounded-full'}`;
