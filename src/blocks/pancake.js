...
export function render(post, postID, config) {
    return buildCard(BLOCK_PANCAKE, post, postID, config, (parts, config) => {
        ...
        return `<article class="@container col-span-1 inline-flex w-full relative" role="article"><div class="flex flex-col w-full ${config.theme.containerBg} h-full ${config.cornerStyle} ${config.interactionClasses}">${parts.finalImageCode}${textContentHTML}</div></article>`;
    });
}
